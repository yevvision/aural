 ...
 */
export const formatDateGerman = (value) => {
    try {
        const safeDate = toSafeDate(value);
 ...
 */
export const formatRelativeTimeGerman = (value) => {
    try {
        const safeDate = toSafeDate(value);
        const now = new Date();
 ...
 */
export const generateWaveformData = async (audioBuffer, samples = 100) => {
    try {
 ...
 */
export const compressAudio = async (file, quality = 0.7) => {
    return new Promise((resolve, reject) => {
        const audio = new Audio();
...
export const createDummyTracks = () => {
    ...
    return [
        ...
        // New track from alex_voice - yuhu
        {
            ...
            userId: (users.find(u => u.username === 'alex_voice') || users[0]).id,
            likes: 15,
            isLiked: false,
            comments: [],
            commentsCount: 0,
        ...
        // Tracks from hollladiewaldfee
        {
            ...
            userId: (users.find(u => u.username === 'hollladiewaldfee') || users[3]).id,
            likes: 23,
            isLiked: false,
            comments: [],
            commentsCount: 5,
 ...
 */
export const sanitizeAudioTrack = (track) => {
    return {
        id: String(track.id || ''),
