...
export const FeedPage = () => {
  ...
  const { tracks, isLoading, setTracks, setLoading, loadTracksFromDatabase } = useFeedStore();
  ...
  // Initialize data from database while preserving existing bookmarks, likes, and including user tracks
  useEffect(() => {
    if (!isInitialized) {
      try {
        console.log('FeedPage: Initialisiere Tracks aus Datenbank...');
        // Lade Tracks aus der Datenbank über den Feed Store
        loadTracksFromDatabase();
        console.log('FeedPage: Tracks geladen, setze isInitialized auf true');
        setIsInitialized(true);
      } catch (error) {
        console.error('Fehler beim Laden der Tracks:', error);
        // Fallback: Setze leere Tracks
        setTracks([]);
        setIsInitialized(true);
      }
    }
  ...
  // Force reload tracks when component mounts to get latest feed data
  useEffect(() => {
    console.log('FeedPage: Component gemountet, lade Tracks...');
    try {
      // Lade Tracks aus der Datenbank
      loadTracksFromDatabase();
      setIsInitialized(true);
    } catch (error) {
      console.error('Fehler beim Neuladen der Tracks:', error);
      setIsInitialized(true);
    }
