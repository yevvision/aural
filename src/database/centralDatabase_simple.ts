...
class CentralDatabaseSimple {
  ...
  updateUser(userId: string, updates: Partial<User>): boolean {
    ...
    this.data.notifications.forEach((notification, index) => {
      if (notification.userId === userId) {
        // Update notification payload if it contains user data
        if (notification.payload && notification.payload.user) {
          this.data.notifications[index] = {
            ...notification,
            payload: {
              ...notification.payload,
              user: {
                ...notification.payload.user,
                ...updates
              }
            }
          };
  ...
  private async loadFromLocalFile(): Promise<void> {
    try {
      // Lade die Datenbank-Datei
      const response = await fetch('/aural_database.json');
      if (!response.ok) {
        throw new Error(`Failed to load database file: ${response.status}`);
      }
      
      const data = await response.json();
      console.log('✅ CentralDB: Loaded data from local file, tracks:', data.tracks?.length || 0);
      
      // Konvertiere Arrays zurück zu Maps
      const likesMap = new Map<string, Set<string>>();
      if (data.likes && Array.isArray(data.likes)) {
        data.likes.forEach((item: { trackId: string; userIds: string[] }) => {
      ...
      if (data.bookmarks && Array.isArray(data.bookmarks)) {
        data.bookmarks.forEach((item: { trackId: string; userIds: string[] }) => {
          if (item && item.trackId && Array.isArray(item.userIds)) {
      ...
      if (data.commentLikesMap && Array.isArray(data.commentLikesMap)) {
        data.commentLikesMap.forEach((item: { commentId: string; userIds: string[] }) => {
      ...
      if (data.playsMap && Array.isArray(data.playsMap)) {
        data.playsMap.forEach((item: { trackId: string; count: number }) => {
      ...
      this.data = {
        ...
        pendingUploads: Array.isArray(data.pendingUploads) ? data.pendingUploads : [],
        follows: Array.isArray(data.follows) ? data.follows : [],
        commentLikes: Array.isArray(data.commentLikes) ? data.commentLikes : [],
        ...
        // User Activities & Notifications
        userActivities: Array.isArray(data.userActivities) ? data.userActivities : [],
        notificationActivities: Array.isArray(data.notificationActivities) ? data.notificationActivities : [],
      ...
      console.log('✅ CentralDB: Data loaded successfully');
    } catch (error) {
      console.error('❌ CentralDB: Error loading from local file:', error);
    }
  }
  ...
  deleteUser(userId: string): boolean {
    try {
      ...
      // Lösche alle Tracks des Users
      this.data.tracks = this.data.tracks.filter(t => t.userId !== userId);
      
      // Lösche alle Kommentare des Users
      this.data.comments = this.data.comments.filter(c => c.user.id !== userId);
      
      // Lösche alle Follows des Users
      this.data.follows = this.data.follows.filter(f => f.followerId !== userId || f.followeeId !== userId);
      
      // Lösche alle User Activities
      this.data.userActivities = this.data.userActivities.filter(a => a.trackId !== userId);
  ...
  searchTracks(query: string): AudioTrack[] {
    if (!query.trim()) return this.data.tracks;
    ...
    return this.data.tracks.filter(track => 
      ...
      track.user?.username?.toLowerCase().includes(searchTerm) ||
  ...
  getPendingUploads(): PendingUpload[] {
    return this.data.pendingUploads;
  }
